...
def evaluate_model(model, aux_params, tokenizer, dataset, split="test", include_rationale=False):
    ...
    for example in dataset[split]:
        ...
        for option in options:
            ...
            with torch.no_grad():
                if aux_params is not None:
                    aux_output = aux_model(**rationale_input)
...
class Predict_LoRA_Linear(nn.Module):
    ...
    def forward(self, input, eval=False):
        ...
        y = torch.stack([
            input[i] @ lora_right[i] @ lora_left[i]
...
def main_accelerate_cot(args):
    ...
    tokenizer, model = load_tokenizer_and_model(args.main_model_path, args)
    ...
    aux_tokenizer, aux_model = load_tokenizer_and_model(args.aux_model_path, args, cutoff=args.aux_cutoff_layer)
